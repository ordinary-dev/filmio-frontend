...
export async function securePut(endpoint: string, payload: object, token?: string) {
    token = token ? token : Cookies.get('access_token')
...
export async function securePostForm(endpoint: string, payload: FormData, token?: string) {
    token = token ? token : Cookies.get('access_token')
